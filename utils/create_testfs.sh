...
function create_fat {
	...
	if [ ! "$copyfrom" == "" ]; then
		...
		cp "$copyfrom/testfs-exFAT$suffix.dd" "$fsdest" 
	else
		...
		copy_files "$fsdest/testfs-fat32$suffix.dd" "$suffix"
				...
				# Create a 512MB exFAT image
		dd if=/dev/zero of="$fsdest/testfs-exFAT$suffix.dd" bs=512 count=5500000
		mkfs.vfat -F 64 "$fsdest/testfs-exFAT$suffix.dd"
		copy_files "$fsdest/testfs-exFAT$suffix.dd" "$suffix"
